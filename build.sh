...
# Include CDT_INSTALL_DIR in CMAKE_PREFIX_PATH
echo "Using EOSIO.CDT installation at: $CDT_INSTALL_DIR"
CMAKE_PREFIX_PATH="${CDT_INSTALL_DIR}:${CMAKE_PREFIX_PATH}"

if [[ ${BUILD_TESTS} == true ]]; then
   # Ensure eosio version is appropriate.
   nodeos-version-check

   # Include EOSIO_INSTALL_DIR in CMAKE_PREFIX_PATH
   echo "Using EOSIO installation at: $EOSIO_INSTALL_DIR"
   CMAKE_PREFIX_PATH="${EOSIO_INSTALL_DIR}:${CMAKE_PREFIX_PATH}"
fi
...
cmake -DBUILD_TESTS=${BUILD_TESTS} -DCMAKE_MODULE_PATH=${CMAKE_PREFIX_PATH} ../
make -j $CPU_CORES
popd &> /dev/null
