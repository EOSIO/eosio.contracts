...
As you just learned the relation between a `system account` and a `system contract`, we are adding here that not all system accounts contain a system contract, but each system account has important roles in the blockchain functionality, as follows:
