...
namespace eosio {
   /**
    * \brief eosio.wrap contract
    * \details The `eosio.wrap` system contract allows block producers to bypass authorization checks or run privileged actions with 15/21 producer approval and thus simplifies block producers superuser actions. It also makes these actions easier to audit.
   ...
   class [[eosio::contract("eosio.wrap")]] wrap : public contract {
      public:
          ...
          * \preconditions Requires authorization of eosio.wrap which needs to be a privileged account.
          * \effects Deferred transaction RAM usage is billed to 'executer'.
