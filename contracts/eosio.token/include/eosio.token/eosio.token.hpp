...
namespace eosio {
   ...
   class [[eosio::contract("eosio.token")]] token : public contract {
      public:
         ...
         /**
          * \details Allows `issuer` account to create a token in supply of `maximum_supply`. If validation is successful a new entry in statstable for token symbol scope gets created.
          *
          * \param issuer The account that creates the token.
          * \param maximum_supply The maximum supply set for the token created.
          *
          * \requires Token symbol has to be valid.
          * \requires Token symbol must not be already created.
          * \requires maximum_supply has to be smaller than the maximum supply allowed by the system: 1^62 - 1.
          * \requires maximum_supply must be positive.
          */
          ...
          */
         [[eosio::action]] void issue( const name& to, 
                                       const asset& quantity, 
                                       const string& memo );

         /**
          * \details The opposite for create action, if all validations succeed, it debits the statstable.supply amount.
          *
          * \param quantity The quantity of tokens to retire.
          * \param memo The memo string to accompany the transaction.
          */
         [[eosio::action]] void retire(   const asset& quantity, 
                                          const string& memo );

         /**
          * \details Allows `from` account to transfer to `to` account the `quantity` tokens.
          * \effects One account is debited and the other is credited with quantity tokens.
          *
          ...
          *
          * \notes More information can be read [here](https://github.com/EOSIO/eosio.contracts/issues/62)
         ...
         /**
          * \details This action is the opposite for open, it closes the account `owner` for token `symbol`.
          *
          * \param owner The owner account to execute the close action for.
          * \param symbol The symbol of the token to execute the close action for.
          *
          * \preconditions The pair of owner plus symbol has to exist otherwise no action is executed.
          * \preconditions If the pair of owner plus symbol exists, the balance has to be zero.
          */
