...
namespace eosio_system
{
   ...
   public:
      ...
      {
         return push_action(name(from), N(rentcpu), mvo()("from", from)("receiver", receiver)("loan_payment", payment)("loan_fund", fund));
      }
      ...
      {
         return push_action(name(bidder), N(bidname), mvo()("bidder", bidder)("newname", newname)("bid", bid));
      }
      ...
      {
         action_result r = push_action(acnt, N(regproducer), mvo()("producer", acnt)("producer_key", get_public_key(acnt, "active"))("url", "")("location", 0));
      ...
      {
         return push_action(voter, N(voteproducer), mvo()("voter", voter)("proxy", proxy)("producers", producers));
      }
      ...
      {
         base_tester::push_action(N(eosio.token), N(issue), manager, mutable_variant_object()("to", manager)("quantity", amount)("memo", ""));
      }

      void transfer(const name &from, const name &to, const asset &amount, const name &manager = config::system_account_name)
      {
         base_tester::push_action(N(eosio.token), N(transfer), manager, mutable_variant_object()("from", from)("to", to)("quantity", amount)("memo", ""));
      }
      ...
      fc::variant get_refund_request(name account)
      {
         ...
         return data.empty() ? fc::variant() : abi_ser.binary_to_variant("refund_request", data, eosio::chain::abi_serializer::create_yield_function(abi_serializer_max_time));
      }
      ...
      abi_serializer initialize_multisig()
      {
         abi_serializer msig_abi_ser;
         {
            ...
            auto trace = base_tester::push_action(config::system_account_name, N(setpriv),
                                                  config::system_account_name, mutable_variant_object()("account", "eosio.msig")("is_priv", 1));
      ...
      vector<name> active_and_vote_producers()
      {
         ...
         //vote for producers
         {
            ...
            BOOST_REQUIRE_EQUAL(success(), push_action(N(alice1111111), N(voteproducer), mvo()("voter", "alice1111111")("proxy", name(0).to_string())("producers", vector<account_name>(producer_names.begin(), producer_names.begin() + 21))));
         }
         produce_blocks(250);
      ...
      void cross_15_percent_threshold()
      {
         ...
         regproducer(N(producer1111));
         {
            ...
            trx.actions.emplace_back(get_action(config::system_account_name, N(voteproducer),
                                                ...
                                                mvo()("voter", "producer1111")("proxy", name(0).to_string())("producers", vector<account_name>(1, N(producer1111)))));
      ...
      {
         return push_action(N(eosio), N(setinflation), mvo()("annual_rate", annual_rate)("inflation_pay_factor", inflation_pay_factor)("votepay_factor", votepay_factor));
      }
   ...
   {
      return mutable_variant_object()("owner", acct)("proxy", name(0).to_string())("producers", variants())("staked", int64_t(0))
